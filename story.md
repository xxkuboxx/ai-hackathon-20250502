...
従来のAI音楽システムの一般的なワークフロー：
```
音声ファイル → STT(Speech-to-Text) → テキスト解析 → 音楽情報抽出
(誤差とレイテンシが累積)
```

**Session MUSEのワークフロー：**
```python
# Gemini 1.5 Proのマルチモーダルプロンプト例
from google.generativeai import GenerativeModel

model = GenerativeModel('gemini-1.5-pro')
response = model.generate_content([
    "この音声ファイルから音楽的情報を解析してください：",
    "キー、BPM、コード進行、ムードをJSON形式で返してください",
    audio_file  # 音声ファイルを直接入力
])
```
...
```python
# バッキングトラック生成のコアロジック（簡略版）
def generate_backing_track(key, bpm, chord_progression, genre):
    # ドラムパターン生成
    drum_pattern = create_drum_pattern(bpm, genre)
    
    # ベースライン生成
    bass_line = create_bass_line(key, chord_progression, genre)
    
    # MIDIデータへ変換
    midi_data = combine_tracks(drum_pattern, bass_line)
    
    # オーディオレンダリング
    return render_to_audio(midi_data, bpm)

# ジャンル別ドラムパターンの例
GENRE_PATTERNS = {
    "rock": [1, 0, 1, 0, 1, 0, 1, 0],  # 4/4 ビート
    "jazz": [1, 0, 0, 1, 0, 1, 0, 0],  # スウィング
    "pop": [1, 0, 1, 0, 1, 0, 1, 0]    # シンプルビート
}
```
...
```mermaid
flowchart TD
    A[🎤 ユーザー] -->|1. 音声アップロード| B[Next.js Frontend]
    B -->|2. ファイル保存| C[Cloud Storage]
    C -->|3. イベント発生| D[Eventarc]
    D -->|4. サービス起動| E[Cloud Run<br/>Python + FastAPI]
    E -->|5. 音声解析| F[Gemini 1.5 Pro]
    F -->|6. 解析結果| E
    E -->|7. バッキング生成| G[🎵 MIDIエンジン]
    G -->|8. オーディオ生成| H[MP3 ファイル]
    H -->|9. 結果返却| B
    B -->|10. ダウンロード| A
...
**1. 鼻歌のアップロード**
```
ユーザー：ふと浮かんだメロディを録音してアップロード
ファイル：humming_melody.mp3 (15秒)
内容：テンポの速いポップ調のメロディ
```
...
**3. バッキングトラック生成**
```
Session MUSE：「こんな感じのバッキングトラックを作ってみました！」
▶️ [backing_track_c_major_128bpm.mp3] (30秒)
ドラム：4/4ビートのシンプルパターン
ベース：C-Am-F-Gの進行に沿ったライン
```

**4. 対話セッション**
```
Takeshi：「もっとドラマチックにできませんか？」

Session MUSE：「ドラマを出すなら、Am部分でマイナーセブンスを使ってみましょう。また、ドラムにシンバルクラッシュを追加して、サビでの高揚感を演出できますよ！」

Takeshi：「雨の日の東京みたいな雰囲気にしたいです」

Session MUSE：「素敵なイメージですね！リバーブを深めにかけて、ディレイで音に余韻を持たせると、湿ったアスファルトのような雰囲気が出ます。コードは、F add9やCsus4を使うとより情緒的になりますよ。」
```
